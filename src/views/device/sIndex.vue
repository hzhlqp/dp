<template>
  <div class="chart-main con-main">
    <div class="con-title">SCBC ACTIVE DEVICES</div>
    <div class="chart-con">
...
export default class eventIndex extends Mixins(
  ...
  chartClick(e: any) {
    ...
    arr.forEach((i: string) => {
      let nArr = this.chartDataList[i].data.rows.forEach((j: any) => {
        let value = e.value;
        ...
        if (e.seriesType === "pie") {
          chartType = "devicesIndexRing4";
        }

        if (j[e.seriesName] === value) {
          chartType = i;
        }
      });
    });
    ...
    // 每个参数对应每个图标类型
    let dict: any = {
      devicesIndexLine1: "area",
      devicesIndexHistogram2: "customer",
      devicesIndexHistogram3: "country",
      devicesIndexRing4: "model"
    };

    let path;
    let chartName = dict[chartType];
    if (Array.isArray(e.value)) {
      path = {
        path: "/device/childsIndex",
        query: {
          dataType: this.filterData.devicesIndexLine1.dataType,
          group: "area",
          date: e.value[0],
          timeType: ""
        }
      ...
      path = {
        path: "/device/childsIndex",
        query: {
          dataType: this.filterData[chartType].dataType,
          timeType: this.filterData[chartType].timeType,
          date: "",
          group: chartName
        }
      };

      path.query![chartName] = e.name;
    }
